...
public class BinaryTree {
	...
	class Pair1 {
		...
		public Pair1(Node currNode, int vertLvl) {
	...
	private void verticalOrder(Node root) {
		...
		Queue<Pair1> bfs = new LinkedList<>();
		
		bfs.add(new Pair1(root, 0));
		Map<Integer, List<Integer>> map = new TreeMap<>();
		while(!bfs.isEmpty()) {
			Pair1 front = bfs.poll();
			Node currNode = front.currNode;
			int currLvl = front.vertLvl;
			
			if(currNode.left != null) {
				bfs.add(new Pair1(currNode.left, currLvl - 1));
			}
			
			if(currNode.right != null) {
				bfs.add(new Pair1(currNode.right, currLvl + 1));
			}
			
			List<Integer> l = map.getOrDefault(currLvl, new ArrayList<>());
			l.add(currNode.val);
			map.put(currLvl, l);
		}
		...
		for(List<Integer> currL : map.values()) {
			for(int e : currL) {
				ans.add(e);
			}
		}
